...
def load_vocab(filename="vocab.txt"):
...
def train_with_sgd(model, indices, processedImages, learning_rate=0.001, nepoch=200, decay=0.9,
    callback_every=10000, callback=None, whichepoch=0):
    num_examples_seen = 0
    for epoch in range(nepoch):
	for i in np.random.permutation(len(indices)):
	  # One SGD step
	  s = model.sgd_step(indices[i], processedImages[i], learning_rate, decay)
	  num_examples_seen += 1
	  # Optionally do callback
	  if (callback and callback_every and num_examples_seen % callback_every == 0):
	    callback(model, num_examples_seen, s, whichepoch, i)            
    return model
...
def load_model_parameters_theano(path, modelClass=GRUTheano):
    ...
    E, U, W, V, b = npzfile["E"], npzfile["U"], npzfile["W"], npzfile["V"], npzfile["b"]
    hidden_dim, word_dim = E.shape[0], E.shape[1]
    print "Building model model from %s with hidden_dim=%d word_dim=%d" % (path, hidden_dim, word_dim)
    sys.stdout.flush()
    model = modelClass(word_dim, hidden_dim=256)
    model.E.set_value(E)
    model.U.set_value(U)
    model.W.set_value(W)
    model.V.set_value(V)
...
def load_model_embeddings_theano(path, modelClass=GRUTheano):
    npzfile = np.load(path)
    E = npzfile["E"]
    hidden_dim, word_dim = E.shape[0], E.shape[1]
    print "Building model model from %s with hidden_dim=%d word_dim=%d" % (path, hidden_dim, word_dim)
